            ...
            while($row = $stmt->fetch()){
                ...
                while($row5 = $stmt5->fetch()){
                    ...
                    while($row5 = $stmt5->fetch()){
                        ...
                        $typeStatus = explode("@", $_GET['tel']);

                        if($typeStatus[1] == "All"){
                            $stmt4 = $pdo->prepare('SELECT * FROM orders WHERE orderID = "'.$row5["orderID"].'" AND orderType="'.$typeStatus[0].'"');
                        }else{
                            $stmt4 = $pdo->prepare('SELECT * FROM orders WHERE orderID = "'.$row5["orderID"].'" AND orderType="'.$typeStatus[0].'" AND orderStatus="'.$typeStatus[1].'"');
                        }
                        ...
                        echo '
                            <tr>
                                ...
                                <td><p><a href="../see_order/index.php?tel='.$row4["orderType"].'_'.$row4["orderID"].'_'.$row4["orderVersion"].'">Zobacz więcej</a></p></td>
                            </tr>';
                            $orderCount++;
                        }
                    }
